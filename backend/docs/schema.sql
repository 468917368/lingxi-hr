...
CREATE TABLE IF NOT EXISTS `sys_user_profile` (
    ...
    `available_from`      DATE     COMMENT '到岗时间：ASAP=随时到岗 1W=1周内 1M=1个月内',
...
CREATE TABLE IF NOT EXISTS `msg_notification` (
    ...
    `type`                VARCHAR(32)     NOT NULL COMMENT '通知类型：APPLICATION=投递通知 INTERVIEW=面试提醒 OFFER=Offer通知 HC_WARNING=HC预警 EVAL_TIMEOUT=评估超时 JOB_REFRESH=岗位刷新 SYSTEM=系统通知',
...
CREATE TABLE IF NOT EXISTS `hr_interview` (
    ...
    `status`              VARCHAR(20)     NOT NULL DEFAULT 'PENDING' COMMENT 'PENDING / SCHEDULED / IN_PROGRESS / COMPLETED / CANCELLED',
    `created_at`          DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `updated_at`          DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
    KEY `idx_company_id` (`company_id`),
...
CREATE TABLE IF NOT EXISTS `hr_offer` (
    ...
    `status`              VARCHAR(16)     NOT NULL DEFAULT 'SENT' COMMENT 'Offer状态：SENT=待确认 ACCEPTED=已确认 REJECTED=已拒绝 EXPIRED=已过期 WITHDRAWN=已撤回',
    ...
    `updated_at`          DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
    -- 2026-08-07 放宽：删除唯一约束，允许 WITHDRAWN/EXPIRED 后再次发起（应用层校验活跃 SENT/终态）
    KEY `idx_application_id` (`application_id`),
    KEY `idx_company_id` (`company_id`),
    KEY `idx_candidate_id` (`candidate_id`),
    KEY `idx_job_id` (`job_id`),
...
CREATE TABLE IF NOT EXISTS `admin_announcement_read` (
    ...
    `user_id`             BIGINT UNSIGNED NOT NULL COMMENT '用户ID',
    `user_type`           VARCHAR(20)     NOT NULL COMMENT '用户类型：CANDIDATE=求职者 HR=HR INTERVIEWER=面试官',
...
CREATE TABLE IF NOT EXISTS `mock_session` (
    ...
    `total_questions`     TINYINT         NOT NULL DEFAULT 5 COMMENT '总题数：5/8/10',
...
ALTER TABLE `resume_ability_model`
    MODIFY COLUMN `professional_skill_score`    TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '专业技能(0-100)',
    MODIFY COLUMN `work_experience_score`       TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '项目与工作经验(0-100)',
    MODIFY COLUMN `industry_knowledge_score`    TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '行业认知(0-100)',
    MODIFY COLUMN `comprehensive_quality_score` TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '综合素质(0-100)',
    MODIFY COLUMN `learning_growth_score`       TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '学习成长(0-100)';

ALTER TABLE sys_user ADD COLUMN name_updated_at DATETIME COMMENT '姓名最后修改时间';
ALTER TABLE sys_user ADD COLUMN first_login TINYINT NOT NULL DEFAULT 0 COMMENT '首次登录标记：0=否 1=是（默认密码未修改）';
ALTER TABLE admin_user ADD COLUMN first_login TINYINT NOT NULL DEFAULT 0 COMMENT '首次登录标记：0=否 1=是（默认密码未修改）';

ALTER TABLE sys_user_profile ADD COLUMN gender VARCHAR(16) COMMENT '性别：MALE/FEMALE';
ALTER TABLE sys_user_profile ADD COLUMN work_years VARCHAR(16) COMMENT '工作年限：FRESH/1-3/3-5/5-10/10+';
ALTER TABLE sys_user_profile ADD COLUMN education VARCHAR(16) COMMENT '最高学历：COLLEGE/BACHELOR/MASTER/PHD';
ALTER TABLE sys_user_profile ADD COLUMN match_notify TINYINT NOT NULL DEFAULT 1 COMMENT '匹配通知：0=关闭 1=开启';
...
CREATE TABLE IF NOT EXISTS `sys_login_log` (
                                               ...
                                               `login_device`        VARCHAR(255)    COMMENT '登录设备/浏览器UA',
                                               `login_location`      VARCHAR(64)     COMMENT '登录地点',
                                               `login_time`          DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '登录时间',
                                               `login_status`        TINYINT         NOT NULL DEFAULT 1 COMMENT '登录状态：1=成功 0=失败',
                                               `fail_reason`         VARCHAR(64)     COMMENT '失败原因',
                                               PRIMARY KEY (`id`),
                                               KEY `idx_user_id` (`user_id`),
                                               KEY `idx_login_time` (`login_time`),
                                               KEY `idx_user_login_time` (`user_id`, `login_time` DESC),
                                               KEY `idx_login_status` (`login_status`)
) ENGINE=InnoDB DEFAULT CHARACTER SET utf8mb4 COMMENT='登录日志表';


ALTER TABLE `resume`
    ADD COLUMN `phone`          VARCHAR(20)  COMMENT '联系电话（用户手动填写，非解析产物）' AFTER `card_structure`,
    ADD COLUMN `email`          VARCHAR(128) COMMENT '联系邮箱（用户手动填写，非解析产物）' AFTER `phone`,
    ADD COLUMN `wechat`         VARCHAR(64)  COMMENT '微信号（用户手动填写，非解析产物）' AFTER `email`,
    ADD COLUMN `candidate_name` VARCHAR(64)  COMMENT '姓名（用户手动填写或解析预填）' AFTER `wechat`;


ALTER TABLE `resume`
    ADD COLUMN `face_photo_url` VARCHAR(512)
        COMMENT '人脸照片MinIO路径'
        AFTER `resume_md_url`;

ALTER TABLE `job_post`
    add COLUMN `industry_group_code` VARCHAR(32) NOT NULL DEFAULT 'OTHER' COMMENT '岗位一级行业编码',
    add COLUMN `industry_code`       VARCHAR(64) NOT NULL DEFAULT 'OTHER' COMMENT '岗位具体行业编码';

-- 6. 系统通知表
-- 系统级别的通知消息，与msg_notification区分
-- -----------------------------------------------------------
CREATE TABLE IF NOT EXISTS `sys_notification` (
                                                  `id`                  BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '通知ID',
                                                  `user_id`             BIGINT UNSIGNED NOT NULL COMMENT '接收用户ID',
                                                  `type`                VARCHAR(32)     NOT NULL COMMENT '通知类型：APPLICATION/INTERVIEW/OFFER/SYSTEM/JOB_RECOMMEND',
                                                  `title`               VARCHAR(100)    NOT NULL COMMENT '通知标题',
                                                  `content`             VARCHAR(500)    COMMENT '通知内容',
                                                  `target_type`         VARCHAR(32)     COMMENT '关联目标类型：application/interview/offer',
...
) ENGINE=InnoDB DEFAULT CHARACTER SET utf8mb4 COMMENT='系统通知表';

ALTER TABLE msg_message
    ADD COLUMN `msg_type` VARCHAR(20) NOT NULL DEFAULT 'TEXT' COMMENT '消息类型：TEXT=纯文本 RICH_TEXT=富文本 IMAGE=图片 FILE=文件';


-- 8. 会话成员表
-- 存储会话中的成员信息，支持置顶、免打扰等功能
-- -----------------------------------------------------------
CREATE TABLE IF NOT EXISTS `msg_conversation_member` (
                                                         `id`                  BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键ID',
                                                         `conversation_id`     BIGINT UNSIGNED NOT NULL COMMENT '会话ID',
                                                         `user_id`             BIGINT UNSIGNED NOT NULL COMMENT '用户ID',
                                                         `member_role`         VARCHAR(16)     NOT NULL COMMENT '成员角色：CANDIDATE/HR',
                                                         `unread_count`        INT             NOT NULL DEFAULT 0 COMMENT '未读消息数',
                                                         `is_top`              TINYINT         NOT NULL DEFAULT 0 COMMENT '是否置顶：0=否 1=是',
                                                         `is_muted`            TINYINT         NOT NULL DEFAULT 0 COMMENT '是否免打扰：0=否 1=是',
                                                         `is_deleted`          TINYINT         NOT NULL DEFAULT 0 COMMENT '是否删除：0=否 1=是（用户自己隐藏）',
                                                         `last_read_at`        DATETIME        COMMENT '最后已读时间',
                                                         `joined_at`           DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '加入时间',
                                                         PRIMARY KEY (`id`),
                                                         UNIQUE KEY `uk_conversation_user` (`conversation_id`, `user_id`),
                                                         KEY `idx_user` (`user_id`, `joined_at` DESC)
) ENGINE=InnoDB DEFAULT CHARACTER SET utf8mb4 COMMENT='会话成员表（公共）';

ALTER TABLE `msg_message`
    ADD COLUMN `media_url` VARCHAR(512) COMMENT '媒体文件URL（图片/文件消息使用，存储MinIO/OSS地址）';

ALTER TABLE `msg_message`
    ADD COLUMN `file_name` VARCHAR(255) COMMENT '原始文件名（文件消息使用）' AFTER `media_url`,
    ADD COLUMN `file_size` BIGINT UNSIGNED COMMENT '文件大小（字节，文件消息使用）' AFTER `file_name`;


ALTER TABLE resume_application
    ADD COLUMN ai_score DECIMAL(5,2) COMMENT 'AI评估分(0-100)',
    ADD COLUMN ai_analysis JSON COMMENT 'AI详细分析';

ALTER TABLE hr_offer DROP INDEX uk_application_id;
ALTER TABLE hr_offer ADD INDEX idx_application_id (application_id);

ALTER TABLE job_post
    ADD INDEX idx_job_public_latest (status, deleted_at, published_at, id),
    ADD INDEX idx_job_public_industry (status, deleted_at, industry_group_code, city_code, published_at, id);

CREATE TABLE IF NOT EXISTS job_city_dict (
                                             code       VARCHAR(32)  PRIMARY KEY COMMENT '城市编码（市辖区/地级市 6 位码，统一编码标准）',
...
ALTER TABLE job_city_dict
    DROP PRIMARY KEY,
    ADD COLUMN id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键' FIRST,
    ADD PRIMARY KEY (id),
    ADD UNIQUE KEY uk_job_city_dict_code (code);

-- 修改表的默认排序规则（会同时修改表中所有字符串字段的排序规则）
ALTER TABLE job_city_dict
    CONVERT TO CHARACTER SET utf8mb4
        COLLATE utf8mb4_unicode_ci;

ALTER TABLE job_post CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
# -- 1. 测试候选人
# INSERT INTO sys_user (phone, password_hash, name, role, status)
# VALUES ('13800000001', '$2b$12$LKVPmwRq2Mtz7v8QhT1f5OTCRg63KHR3xeg3R5z0m6sFBHRNZ/QRy', '测试候选人', 'CANDIDATE', 'ACTIVE');
# -- 记下 userId
#
# -- 2. 岗位
# INSERT INTO job_post (company_id, title, city_code, city_name, min_experience_years, education_requirement, status, created_by)
# VALUES (1, '高级Java工程师', '110100', '北京', 3, 'BACHELOR', 'PUBLISHED', 1);
# -- 记下 jobId
#
# -- 3. 岗位画像（requirements 接口数据源，job_type 必填）
# INSERT INTO job_profile (company_id, job_id, job_type, core_skills, soft_skills, interview_focus, profile_source)
# VALUES (1, <jobId>, 'JAVA_BACKEND',
#         '[{"name":"Spring Boot","level":"精通","required":true},{"name":"MySQL","level":"熟练","required":true}]',
#         '[{"name":"沟通协作","importance":"重要"}]',
#         '["并发编程","JVM调优","分布式事务"]',
#         'MANUAL');
#
# -- 4. 简历（candidate_id = userId）
# INSERT INTO resume (candidate_id, file_name, file_url, file_format, file_size, is_default, parse_status, card_structure)
# VALUES (<userId>, '测试简历.pdf', 'oss://test/resume.pdf', 'pdf', 1024, 1, 'COMPLETED',
#          '{"cards":[{"id":"c1","title":"技能","points":[{"id":"p1","text":"Java,Spring Boot"}]},{"id":"c2","title":"项目","points":[{"id":"p2","text":"电商平台订单模块"}]}]}');
# -- 记下 resumeId
#
# -- 5. 能力模型（可选但推荐，出题难度调节依据）
# INSERT INTO resume_ability_model (candidate_id, resume_id, professional_skill_score, work_experience_score, industry_knowledge_score, comprehensive_quality_score, learning_growth_score, sub_dimensions)
# VALUES (<userId>, <resumeId>, 72, 68, 65, 70, 75,
#          '{"skills":["Java","Spring","MySQL"],"workYears":"3年"}');


-- resume_application 是 AUTO_INCREMENT，不用手写 id；两条用不同 candidate 避开 uk_job_candidate
INSERT INTO resume_application (job_id, candidate_id, resume_id, status, match_score, submitted_at)
VALUES
    (2, 18, 1,   'SUBMITTED', 92.00, NOW()),
    (2, 1,  NULL, 'VIEWED',    75.50, NOW());

UPDATE resume_application SET status='SCREENED', screened_at=NOW() WHERE id=50003;

UPDATE hr_interview SET status='SCHEDULED' WHERE status='CONFIRMED';

UPDATE hr_offer SET status='REJECTED', rejected_at=NOW() WHERE id=60102;

UPDATE job_post SET total_hc = 10, updated_at = NOW() WHERE id BETWEEN 2 AND 11;

INSERT INTO job_city_dict (code, name, status) VALUES
                                                   ('110100', '北京', 1),
                                                   ('310100', '上海', 1),
                                                   ('330100', '杭州', 1),
                                                   ('440300', '深圳', 1),
