/**
 * 修改手机号弹窗：新手机号 + 验证码（60s 倒计时），成功后 token 失效重新登录。
 */
import React, { useState, useRef, useEffect } from 'react';
import { Form, Input, Button, Modal, message } from 'antd';
import { sendHrPhoneCode, changeHrPhone } from '@/services/hr';
...
const PhoneModal = ({ visible, currentPhone, onClose, onSuccess }: PhoneModalProps) => {
  ...
  const handleSendCode = async () => {
    ...
    setSending(true);
    try {
      await sendHrPhoneCode(newPhone);
  ...
  /** 提交修改（成功即 token 失效，需重新登录） */
  const handleSubmit = async () => {
    try {
      const values = await form.validateFields();
      setSubmitting(true);

      await changeHrPhone(values.newPhone, values.code);
